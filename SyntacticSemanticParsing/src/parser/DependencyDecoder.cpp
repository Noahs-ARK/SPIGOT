...
void DependencyDecoder::Decode(Instance *instance, Parts *parts,
	...
	if (dependency_parts->IsArcFactored() ||
		...
		DecodeFactorGraph(instance, parts, copied_scores, false, true,
		                  predicted_output);
#endif
		// At test time, run Chu-Liu-Edmonds on top of the outcome of ARGMAX_STE
		// as a rounding heuristic to make sure we get a valid tree.
		// TODO: maybe change the interface to ARGMAX_STE to let us implement
...
void DependencyDecoder::DecodeBasic(Instance *instance, Parts *parts,
	...
	vector<int> heads;
	if (pipe_->GetSemanticOptions()->projective()) {
		RunEisner(slen, arcs, scores_arcs, &heads, value);
	} else {
		RunChuLiuEdmonds(slen, arcs, scores_arcs, &heads, value);
	}
...
// tree.
void DependencyDecoder::RunChuLiuEdmonds(int slen,
                                         const vector<DependencyPartArc *> &arcs,
                                         const vector<double> &scores,
                                         vector<int> *heads,
                                         double *value) {
	vector<vector<int> > candidate_heads(slen);
	vector<vector<double> > candidate_scores(slen);
	vector<bool> disabled(slen, false);
...
// tree.
void DependencyDecoder::RunEisner(int slen,
                                  const vector<DependencyPartArc *> &arcs,
                                  const vector<double> &scores,
                                  vector<int> *heads,
                                  double *value) {
	...
	vector<vector<double> > complete_spans(slen, vector<double>(
			slen, 0.0));
	vector<vector<int> > complete_backtrack(slen,
	                                        vector<int>(slen, -1));
	...
	for (int s = 1; s < slen; ++s) {
		...
		if (arc_index >= 0) {
			double val = complete_spans[s][1] + complete_spans[s][slen - 1] +
			             scores[arc_index];
			if (best < 0 || val > best_value) {
				best = s;
				best_value = val;
...
void DependencyDecoder::RunEisnerInside(
	...
	for (int s = 0; s < slen; ++s) {
		(*inside_complete_spans)[s].assign(slen, 0.0);
	}
...
//					LOG(INFO) << (*inside_complete_spans)[s][u]<<" "<< (*inside_complete_spans)[t][u + 1];
				}
//				LOG(INFO) << sum.logabs();
//				LOG(INFO) << endl;
				if (left_arc_index >= 0) {
...
//			LOG(INFO) << "inside_complete[" << s << "][" << t << "] = " << (*inside_complete_spans)[s][t];
	...
	(*inside_complete_spans)[0][slen - 1] = sum.logabs();
//	LOG(INFO) << "inside_complete[" << 0 << "][" << slen-1 << "] = " << (*inside_complete_spans)[0][slen-1];
...
void DependencyDecoder::RunEisnerOutside(
	...
	for (int s = 0; s < slen; ++s) {
		(*outside_complete_spans)[s].assign(slen, 0.0);
	}
...
//	CHECK(false) << "Not implemented";
	int slen =
			static_cast<DependencyInstanceNumeric *>(instance)->size() - 1;
	DependencyParts *dependency_parts = static_cast<DependencyParts *>(parts);

	// Matrix for storing the potentials.
	Eigen::MatrixXlogd potentials(slen, slen);
	// Kirchhoff matrix.
	Eigen::MatrixXlogd kirchhoff(slen - 1, slen - 1);
...
//				LOG(INFO) << potentials(m, h).as_float() <<" " << potentials(m, h).logabs();
			}
		}
	}

	// Set the Kirchhoff matrix.
	for (int h = 0; h < slen - 1; ++h) {
		for (int m = 0; m < slen - 1; ++m) {
			kirchhoff(h, m) = -potentials(m + 1, h + 1);
			LOG(INFO) << kirchhoff(h, m).as_float() << " "<<kirchhoff(h, m).logabs() <<" "<< kirchhoff(h, m).signbit();
		}
	}
	for (int m = 1; m < slen; ++m) {
		LogValD sum = LogValD::Zero();
		for (int h = 0; h < slen; ++h) {
//			LOG(INFO)<< potentials(m, h).as_float() <<" " << potentials(m, h).logabs();
//			LOG(INFO) << sum.as_float() <<" " << sum.logabs() <<" "
//			          << potentials(m, h).as_float() <<" " << potentials(m, h).logabs();
			sum += potentials(m, h);
//			LOG(INFO) << sum.as_float() <<" " << sum.logabs();
//			LOG(INFO) << endl;
		}
...
//	}
	...
	*log_partition_function = lu.determinant().logabs() +
	                          constant * (slen - 1);
//	LOG(INFO) << *log_partition_function <<" " << lu.determinant().as_float();
	CHECK(!std::isnan(*log_partition_function));
//	LOG(INFO) << *log_partition_function;
//	LOG(INFO) <<endl;
//	for (int i = 0;i < slen - 1;++ i) {
//		for (int j = 0;j < slen - 1; ++ j) {
//			cout <<inverted_kirchhoff(i, j).as_float() <<" ";
//		}
//		cout <<endl;
//	}
//	LOG(INFO) << endl;
	Eigen::MatrixXlogd marginals(slen, slen);
	for (int h = 0; h < slen; ++h) {
		marginals(0, h) = LogValD::Zero();
	}
	for (int m = 1; m < slen; ++m) {
		marginals(m, 0) = potentials(m, 0) * inverted_kirchhoff(m - 1, m - 1);
//		LOG(INFO) << potentials(m, 0).as_float() <<" "
//		          << inverted_kirchhoff(m - 1, m - 1).as_float() <<" "
//		          << marginals(m, 0).as_float();
		for (int h = 1; h < slen; ++h) {
			marginals(m, h) = potentials(m, h) *
			                  (inverted_kirchhoff(m - 1, m - 1) - inverted_kirchhoff(m - 1, h - 1));
//			LOG(INFO) << marginals(m, h).as_float();
//			LOG(INFO) << inverted_kirchhoff(m - 1, m - 1).as_float()<<" "
//			          << inverted_kirchhoff(m - 1, h - 1).as_float() <<" "
//			          << (inverted_kirchhoff(m - 1, m - 1) - inverted_kirchhoff(m - 1, h - 1)).as_float();
		}
	}
...
// Decode building a factor graph and calling the ARGMAX_STE algorithm.
void DependencyDecoder::DecodeFactorGraph(Instance *instance, Parts *parts,
	...
	if (use_flows) {
		if (prune_paths) {
			graph_paths.assign(slen, vector<bool>(slen, false));
			for (int r = 0; r < num_arcs; ++r) {
	...
	if (use_tree_factor) {
		...
		bool projective = pipe_->GetSemanticOptions()->projective();
		factor->Initialize(projective, slen, arcs, this);
	...
	if (use_flows) {
		...
		vector<vector<vector<AD3::BinaryVariable *> > > local_variables_path_builder(
				slen, vector<vector<AD3::BinaryVariable *> >(slen));
		for (int r = 0; r < num_arcs; ++r) {
			DependencyPartArc *arc =
					static_cast<DependencyPartArc *>((*dependency_parts)[offset_arcs + r]);
			int m = arc->modifier();
			for (int k = 0; k < slen; ++k) {
				int index = offset_flow_variables + k + r * slen;
				local_variables_path_builder[m][k].push_back(variables[index]);
			}
		}
		for (int d = 0; d < slen; ++d) {
			for (int a = 1; a < slen; ++a) {
				int index = offset_path_variables + a + d * slen;
	...
	if (use_trigram_head_automata && num_trisiblings > 0) {
		...
		vector<vector<DependencyPartTriSibl *> > left_trisiblings(slen);
		vector<vector<DependencyPartTriSibl *> > right_trisiblings(slen);
		vector<vector<double> > left_scores(slen);
		vector<vector<double> > right_scores(slen);
		vector<vector<int> > left_indices(slen);
		vector<vector<int> > right_indices(slen);
	...
	if (use_head_automata || use_grandparent_head_automata) {
		...
		vector<vector<DependencyPartGrandpar *> > left_grandparents(slen);
		vector<vector<DependencyPartGrandpar *> > right_grandparents(slen);
		vector<vector<double> > left_grandparent_scores(slen);
		vector<vector<double> > right_grandparent_scores(slen);
		vector<vector<int> > left_grandparent_indices(slen);
		vector<vector<int> > right_grandparent_indices(slen);
		if (use_grandparent_head_automata &&
		...
		vector<vector<DependencyPartGrandSibl *> > left_grandsiblings(slen);
		vector<vector<DependencyPartGrandSibl *> > right_grandsiblings(slen);
		vector<vector<double> > left_grandsibling_scores(slen);
		vector<vector<double> > right_grandsibling_scores(slen);
		vector<vector<int> > left_grandsibling_indices(slen);
		vector<vector<int> > right_grandsibling_indices(slen);
		if (use_grandparent_head_automata &&
		...
		vector<vector<DependencyPartNextSibl *> > left_siblings(slen);
		vector<vector<DependencyPartNextSibl *> > right_siblings(slen);
		vector<vector<double> > left_scores(slen);
		vector<vector<double> > right_scores(slen);
		vector<vector<int> > left_indices(slen);
		vector<vector<int> > right_indices(slen);
		...
		vector<vector<vector<AD3::BinaryVariable *> > >
				local_variables_omega_normalizer(slen,
				                                 vector<vector<AD3::BinaryVariable *> >(slen));

		// Add the rho variables to the arrays.
		for (int h = 0; h < slen; ++h) {
			for (int k = 0; k < slen; ++k) {
				int index = offset_rho_variables + h *slen + k;
			...
			if (h < m) {
				for (int k = m; k < slen; ++k) {
					...
					if (k == m) {
						...
						index = offset_omega_variables + r * slen + k;
					}
					local_variables_omega_normalizer[h][k].push_back(variables[index]);
				}
			} else {
		...
		vector<vector<vector<AD3::BinaryVariable *> > > local_variables_omega_propagate(
				num_arcs, vector<vector<AD3::BinaryVariable *> >(slen));
		vector<vector<vector<bool> > > negated_omega_propagate(
				num_arcs, vector<vector<bool> >(slen));
			...
			if (h < m) {
				for (int k = m + 1; k < slen; ++k) {
					...
					if (k - 1 == m) {
						...
						index = offset_omega_variables + r * slen + k - 1;
					}
					local_variables_omega_propagate[r][k].push_back(variables[index]);
		...
		for (int r = 0; r < num_next_siblings; ++r) {
			...
			if (s == slen) {
				...
				if (h == m) {
					...
					index = offset_rho_variables + h * slen +
					        (slen - 1);
				...
				if (h == m) {
					...
					index = offset_rho_variables + h * slen + 0;
					local_variables[1] = variables[index];
	...
	for (int r = 0; r < num_nonprojective; ++r) {
		...
		for (int k = left; k <= right; ++k) {
			...
			int index = offset_path_variables + part->head() + k * slen;
			local_variables.push_back(variables[index]);
	...
	if (use_head_bigram_sequence_factor && use_head_bigram_parts) {
		...
		vector<AD3::BinaryVariable *> local_variables;
		vector<int> num_states(slen - 1, 0);
		vector<vector<int> > index_heads(slen - 1,
		                                 vector<int>(slen, -1));
		for (int m = 1; m < slen; ++m) {
			for (int h = 0; h < slen; ++h) {
				int r = dependency_parts->FindArc(h, m);
		...
		vector<vector<vector<int> > > index_edges(slen);
		int index = 0;
		for (int i = 0; i < slen; ++i) {
			...
			int num_current_states = (i < slen - 1) ? num_states[i] : 1;
		...
		for (int r = 0; r < num_bigrams; ++r) {
			...
			int current_state = (m == slen) ? 0 : index_heads[m - 1][part->head()];
			CHECK_GE(previous_state, 0);
			if (m > 1) CHECK_LT(previous_state, num_states[m - 2]);
			CHECK_GE(current_state, 0);
			if (m < slen) CHECK_LT(current_state, num_states[m - 1]);
  ...
  stream.close();
	...
	gettimeofday(&end, NULL);
	double elapsed_time = diff_ms(end, start);
	VLOG(2) << "Elapsed time (ARGMAX_STE) = " << elapsed_time
	        << " (" << slen << ") ";
	...
	*value = 0.0;
	...
	VLOG(2) << "Solution value (ARGMAX_STE) = " << *value;
}
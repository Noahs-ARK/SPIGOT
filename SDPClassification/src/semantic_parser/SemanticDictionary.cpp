...
void
SemanticDictionary::CreatePredicateRoleDictionaries(SemanticReader *reader) {
	...
	SemanticInstance *instance =
			dynamic_cast<SemanticInstance *>(reader->GetNext());
	...
	while (instance != NULL) {
		for (int k = 0; k < instance->GetNumPredicates(); ++k) {
			...
			for (int l = 0; l < instance->GetNumArgumentsPredicate(k); ++l) {
				int role_id = role_alphabet_tmp.Insert(
						instance->GetArgumentRole(k, l));
		...
		delete instance;
		instance = dynamic_cast<SemanticInstance *>(reader->GetNext());
	}
	reader->Close();
	...
	LOG(INFO) << "Role alphabet after pruning: " << role_alphabet_.size()
	          << endl;
	...
	bool allow_unseen_predicates =
			dynamic_cast<SemanticPipe *>(pipe_)->GetSemanticOptions()->
	...
	while (instance != NULL) {
		...
		for (int k = 0; k < instance->GetNumPredicates(); ++k) {
			...
			int predicate_pos_id = token_dictionary_->GetPosTagId(
					predicate_pos);
			...
			for (int l = 0; l < instance->GetNumArgumentsPredicate(k); ++l) {
				...
				int argument_pos_id = token_dictionary_->GetPosTagId(
						argument_pos);
				if (argument_pos_id < 0) argument_pos_id = TOKEN_UNKNOWN;
				int role_id = role_alphabet_.Lookup(
						instance->GetArgumentRole(k, l));
				if (role_id < 0)
				...
				for (int m = l + 1;
				     m < instance->GetNumArgumentsPredicate(k); ++m) {
					int other_role_id =
							role_alphabet_.Lookup(
									instance->GetArgumentRole(k, m));
					//CHECK_GE(other_role_id, 0);
					if (other_role_id < 0)
						other_role_id = index_role_unk;
					int bigram_label = GetRoleBigramLabel(role_id,
					                                      other_role_id);
				...
				string pos_path;
				ComputeDependencyPath(instance, p, a, &relation_path,
				                      &pos_path);
				int relation_path_id = relation_path_alphabet.Insert(
						relation_path);
				...
				if (relation_path_id >=
				    existing_roles_with_relation_path.size()) {
					existing_roles_with_relation_path.resize(
							relation_path_id + 1);
				}
	...
	while (true) {
		...
		for (Alphabet::iterator iter = relation_path_alphabet.begin();
			...
			if (relation_path_freqs[iter->second] > relation_path_cutoff) {
				int relation_path_id = relation_path_alphabet_.Insert(
						iter->first);
		...
		if (relation_path_alphabet_.size() < kMaxRelationPathAlphabetSize)
			break;
...
void SemanticTokenDictionary::Initialize(Reader *reader) {
	...
	// counting the frequencies.
	auto semantic_reader = static_cast<SemanticReader *> (reader);
	semantic_reader->Open(semantic_options->GetTrainingFilePath("parser"));
	auto parser_instance =
			dynamic_cast<SemanticInstance *>(semantic_reader->GetNext());
	while (parser_instance != NULL) {
		int instance_length = parser_instance->size();
		for (int i = 0; i < instance_length; ++i) {
			int id;

			// Add form to alphabet.
			std::string form = parser_instance->GetForm(i);
			...
			std::string lemma = parser_instance->GetLemma(i);
			...
			// Add POS to alphabet.
			id = pos_alphabet.Insert(parser_instance->GetPosTag(i));
			...
			id = cpos_alphabet.Insert(parser_instance->GetCoarsePosTag(i));
		...
		parser_instance = dynamic_cast<SemanticInstance *>(semantic_reader->GetNext());
	}
	semantic_reader->Close();

	{
		ifstream in(dynamic_cast<SemanticPipe *> (pipe_)->GetSemanticOptions()
				            ->GetEmbeddingFilePath("parser"));

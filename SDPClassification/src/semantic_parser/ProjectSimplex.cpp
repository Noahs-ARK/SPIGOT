...
void ProjectSDP(Instance *instance, Parts *parts,
	...
	for (int i = 0;i < num_arcs; ++i) {
		int a_var_idx = i + offset_var_arcs;
		auto arc = static_cast<SemanticPartArc *> ((*parts)[i + offset_arcs]);
		const vector<int> &index_labeled_parts =
				semantic_parts->FindLabeledArcs(arc->predicate(),
				                                arc->argument(),
				                                arc->sense());
		int num_la_parts = index_labeled_parts.size();
		if (target[a_var_idx] <= 0) {
			target[a_var_idx] = 0.0;
			for (int j = 0;j < num_la_parts; ++ j) {
				int la_idx = index_labeled_parts[j];
				int la_var_idx = la_idx - offset_labeled_arcs
				                 + offset_var_labeled_arcs;
				target[la_var_idx] = 0.0;
			}
		} else {
			vector<dynet::real> x(num_la_parts, 0.0);
			vector<dynet::real> costs(num_la_parts, 1.0);
			for (int j = 0;j < num_la_parts; ++ j) {
				int la_idx = index_labeled_parts[j];
				int la_var_idx = la_idx - offset_labeled_arcs
				                 + offset_var_labeled_arcs;
				x[j] = target[la_var_idx];
			}
			if (target[a_var_idx]  > 1) {
				target[a_var_idx] = 1.0;
			}
			ProjectOntoKnapsackConstraint(x, costs, num_la_parts, target[a_var_idx]);
			for (int j = 0;j < num_la_parts; ++ j) {
				int la_idx = index_labeled_parts[j];
				int la_var_idx = la_idx - offset_labeled_arcs
				                 + offset_var_labeled_arcs;
				target[la_var_idx] = x[j];
			}
		}
	}
	for (int i = 0;i < num_variables; ++ i) {

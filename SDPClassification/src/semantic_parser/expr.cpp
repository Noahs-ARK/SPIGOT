...
namespace dynet {
	...
	Expression argmax_proj_singlehead(const Expression &x, const Expression &p,
		...
		return Expression(x.pg, x.pg->add_function<ArgmaxProjSingleHead> (
				{x.i, p.i}, instance, parts));
	}

	Expression argmax_proj_sdp(const Expression &x, const Expression &p,
	                                  Instance *instance, Parts *parts) {
		return Expression(x.pg, x.pg->add_function<ArgmaxProjSDP> (
				{x.i, p.i}, instance, parts));
	}
